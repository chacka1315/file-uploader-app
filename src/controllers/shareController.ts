...
const create_share_get: RequestHandler = async (req, res) => {
...
const create_share_post: RequestHandler = async (req, res, next) => {
  let id = req.params.id;
...
const read_share_get: RequestHandler = async (req, res, next) => {
  const id = req.params.shareId;
  let shareId = id.toString();
  try {
    ...
    sharedFolder.folder.files.forEach((file) => {
      const date = new Date(file.uploadedAt).toLocaleDateString();
      const size = (file.size / (1024 * 1024)) * 100;
      file.size = Math.round(size) / 100;
      (file as any).uploadedAt = date;
    });

    const timeLeft = formatDistanceStrict(
      new Date(sharedFolder.sharedAt),
      new Date(sharedFolder.expiresAt),
      { unit: 'day' },
    );

    const share = { ...sharedFolder, timeLeft };
...
const viewSharelink_get: RequestHandler = async (req, res, next) => {
